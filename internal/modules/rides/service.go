...
type service struct {
	...
	wsHelper        *RideWebSocketHelper // Add this line
}

// Update the NewService function
func NewService(
	repo Repository,
	driversRepo driversrepo.Repository,
	ridersRepo ridersrepo.Repository,
	...
	return &service{
		...
		wsHelper:        NewRideWebSocketHelper(), // Add this line
	}
}
...
func (s *service) CreateRide(ctx context.Context, riderID string, req dto.CreateRideRequest) (*dto.RideResponse, error) {
	...
	websocketutil.SendToUser(riderID, websocket.TypeSystemMessage, map[string]interface{}{
		"type":            "ride_searching",
		"rideId":          rideID,
...
func (s *service) FindDriverForRide(ctx context.Context, rideID string) error {
	...
	if err != nil || nearbyDrivers.Count == 0 {
		...
		if err != nil || nearbyDrivers.Count == 0 {
			// No drivers found - notify rider via WebSocket
			websocketutil.SendToUser(ride.RiderID, websocket.TypeSystemMessage, map[string]interface{}{
	...
	for i := 0; i < maxAttempts; i++ {
		...
		websocketutil.SendRideRequest(driver.DriverID, map[string]interface{}{
			"rideId":    rideID,
			"requestId": requestID,
...
func (s *service) AcceptRide(ctx context.Context, driverID, rideID string) (*dto.RideResponse, error) {
	...
	*ride.AcceptedAt = time.Now()
	...
	websocketutil.SendRideAccepted(ride.RiderID, map[string]interface{}{
		"rideId":   rideID,
		"driverId": driver.ID,
		...
		"eta":     10, // Estimated time to pickup
	})
