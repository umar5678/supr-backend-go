...
func (s *service) GetDashboard(ctx context.Context, userID string) (*driverdto.DriverDashboardResponse, error) {
	...
	if err := s.db.WithContext(ctx).
		Model(&models.Ride{}).
		Where("driver_id = ? AND created_at >= ? AND created_at < ? AND status = ?", driver.ID, todayStart, todayEnd, "completed").
		Count(&todayTrips).Error; err != nil {
		logger.Error("failed to query today's trips", "error", err, "driverID", driver.ID)
		todayTrips = 0
	}
	...
	if err := s.db.WithContext(ctx).
		...
		Row().
		Scan(&todayEarnings); err != nil {
		logger.Error("failed to query today's earnings", "error", err, "driverID", driver.ID)
		todayEarnings = 0
	}
	...
	if err := s.db.WithContext(ctx).
		Model(&models.Ride{}).
		Where("driver_id = ? AND created_at >= ? AND created_at < ? AND status = ?", driver.ID, weekStart, todayEnd, "completed").
		Select("COALESCE(SUM(final_fare), 0)").
		Row().
		Scan(&weekEarnings); err != nil {
		logger.Error("failed to query week's earnings", "error", err, "driverID", driver.ID)
		weekEarnings = 0
	}
...
func (s *service) UpdateLocation(ctx context.Context, userID string, req driverdto.UpdateLocationRequest) error {
	...
	if err != nil {
		logger.Debug("No active ride in cache",
			"driverProfileID", driver.ID,
...
func (s *service) TopUpWallet(ctx context.Context, userID string, req driverdto.WalletTopUpRequest) (*driverdto.WalletTopUpResponse, error) {
	...
	paymentResult, err := s.ProcessPayment(ctx, &driverdto.PaymentRequest{
		DriverID:      driver.ID,
		UserID:        userID,
		Amount:        req.Amount,
		PaymentMethod: req.PaymentMethod,
...
func (s *service) ProcessPayment(ctx context.Context, payReq *driverdto.PaymentRequest) (*driverdto.PaymentResult, error) {
	...
	*/
	...
	return &driverdto.PaymentResult{
		Success:       true,
		OrderID:       mockOrderID,
		TransactionID: mockTxnID,
